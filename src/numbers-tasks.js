...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorsLen =
    Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(y2 ** 2 + x2 ** 2);

  return Math.acos(scalar / vectorsLen);
}
...
function getCountOfOddNumbers(number) {
  ...
  const absoluteNumVal = Math.abs(number);

  for (let i = 1; i <= absoluteNumVal; i += 1) {
    if (i % 2 !== 0) odd += 1;
  }
